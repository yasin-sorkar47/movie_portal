...
export default function MovieDetailsModal({ onClose, movie, onAddToCard }) {
  const { darkMode } = useContext(themeContext);
  return (
    <div className="fixed top-0 left-0 w-screen h-screen z-50 bg-black/30 backdrop-blur-sm">
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[984px] p-4 max-h-[90vh] overflow-auto">
        <div
          className={` shadow-md ${
            darkMode ? "dark:bg-[#12141D]" : "bg-white"
          } rounded-2xl sm:grid sm:grid-cols-[2fr_1fr] overflow-hidden`}
        >
          <img
            ...
            <div className="grid lg:grid-cols-2 gap-2">
              ...
              <a
                className={`border border-[#74766F] rounded-lg py-2 px-5 flex items-center justify-center gap-2 ${
                  darkMode ? "dark:text-gray-200" : "text-[#6F6F6F]"
